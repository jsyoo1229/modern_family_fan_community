...
## 목표와 기능
### 목표 ###
미국 시트콤 모던 패밀리의 팬 커뮤니티 사이트로, 사용자에게 게시글을 작성하고, 댓글을 달며, 좋아요와 스크랩 기능을 통해 자신만의 커뮤니티 경험을 제공합니다.
...
## 느낀점
* 기본에 대한 이해
좋아요와 스크랩 기능을 구현하는데 DRF view와 모델의 기본에 대한 이해가 필수적이었습니다.
상속 받은 클래스에 내장된 함수들을 이용하여 커스터마이징 하는 코드가 많았는데,
어떤 내장 함수가 존재하는지, 모델의 어떤 필드를 인자로 전해줘야 하는지 등의, 기본을 알아야만
구현이 가능했습니다. 

* CICD 실패
프로젝트 완료 후, Github Actions와 AWS EC2 인스턴스로 배포에도 도전해보았지만,
ssh 인증 과정에서 오류가 발생해 배포에 실패했습니다.
상당히 아쉬웠지만, 배포에 도전해보면서, 가상화, 도커, 컨테이너라는 개념에 대해
학습하게 된 좋은 계기였고, 리눅스 코드도 직접 경험해볼 수 있어 좋았습니다.
